...
public final class ExpandQueryStrategy2 implements Strategy2<FocusedSolverState<CResolveQuery>, SolverState, SolverContext> {

    @Override
    public List<SolverState> apply(SolverContext ctx, FocusedSolverState<CResolveQuery> input) throws InterruptedException {
        ...
        List<SolverState> output = IntStream.range(0, count.get()).mapToObj(idx -> {
            ...
            return IntStream.rangeClosed(resultSize.lowerEndpoint(), resultSize.upperEndpoint())
                .mapToObj(size -> StreamUtils.subsetsOfSize(optMatches.stream(), size).map(matches -> {
                    ...
                    ImmutableList<IConstraint> newConstraints = removeTermIdConstraints(constraints.build());

                    return input.getInnerState().updateConstraints(newConstraints, Iterables2.singleton(query));
